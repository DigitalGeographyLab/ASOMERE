"""
Script for producing Figure 4. 
Temporal patterns and correlations between official visitor statistics and social media user-days in Finnish national parks.
...
fin_name_convert = {'Aulangon luonnonsuojelualue': 'Aulanko',
                'Etelä-Konneveden kansallispuisto': 'Etelä-Konnevesi',
                'Helvetinjärven kansallispuisto': 'Helvetinjärvi',
                'Hiidenportin kansallispuisto': 'Hiidenportti',
                'Isojärven kansallispuisto': 'Isojärvi',
                'Itäisen Suomenlahden kansallispuisto': 'It. Suomenlahti',
                'Kauhanevan-Pohjankankaan kansallispuisto': 'Kauha.-Pohjank.',
                ...
                'Linnansaaren kansallispuisto': 'Linnansaari',
                'Nuuksion kansallispuisto': 'Nuuksio',
                'Oulangan kansallispuisto': 'Oulanka',
                'Pallas-Yllästunturin kansallispuisto': 'Pallas-Ylläs',
                'Patvinsuon kansallispuisto': 'Patvinsuo',
                'Perämeren kansallispuisto': 'Perämeri',
                'Petkeljärven kansallispuisto': 'Petkeljärvi',
                'Punkaharjun luonnonsuojelualue': 'Punkaharju',
                ...
                'Tammisaaren saariston kansallispuisto': 'Tammisaari',
                'Teijon kansallispuisto': 'Teijo',
...
# Data folder
fold = "/home/social-media/FI"

# Filepaths
flickrfp = os.path.join(fold, "Flickr_Finland_NPs_2008-2016_withinPark_OceanicBuffer5km_withParkNames.shp")
instafp = os.path.join(fold, "Instagram_Finland_NPs_1km_buffer_2013-2016_with_ParkNames.shp")
twitterfp = os.path.join(fold, "DOLLY_FIN_NPs_tweets_2012-2016_WithinPark_OceanicBuffer5km_withParkNames.shp")
statsfp = "Finland_Official_NP_VisitorStatistics_AllPark_essentialVariables.csv"
outputdir = "/home/results/FI"
...
# There seems to be some duplicates in stats ==> remove
stats = stats.drop_duplicates()

# Rename columns
twitter = twitter.rename(columns={'u_id': 'userid', 'id': 'photoid', 'Nimi': 'park'})
insta = insta.rename(columns={'Nimi': 'park'})
flickr = flickr.rename(columns={'Nimi': 'park'})

# Select relevant columns from social media datasets
cols = ['park', 'time_local', 'userid', 'photoid', 'Distance', 'geometry']
insta = insta[cols]
flickr = flickr[cols]
...
# Rename visitor statistics names for Finland (make shorter)
join['park'] = join['park'].replace(to_replace=fin_name_convert)
...
# Plot the temporal patterns of all Finnish national parks as subplots
# ====================================================================

fig, axarray = plt.subplots(nrows=6, ncols=6, figsize=(17,14.35), sharey=True)
...
for i, park in enumerate(sel_parks):
    ...
    # ---------------------------------
    rows = normalizeColumns(df=rows, src_cols=['kayntimaara'], target_cols=['Official'])
    ...
    # Save the legend in the last round ('Kolovesi is the last park plotted')
    if park == 'Kolovesi': 
        lgnd = ax
...
# Visualize the last axis
ax = axarray[-1][-1]
ax.grid('off')
ax.axis('off')
lgnd_font_s = 14
lgnd.legend(bbox_to_anchor=(1.1, 1.1), prop={'size':lgnd_font_s, 'family': 'Arial'})
...
plgnd_y = dlgnd_y-0.335
ax.text(0.14, plgnd_y, "r", **{'size':14, 'family': 'Arial'})
ax.text(0.303, plgnd_y, "Pearson's correlation", **{'size':14, 'family': 'Arial'})

# Add legend for Park ID
ax.text(0.12, plgnd_y-0.09, "ID", **{'size':14, 'family': 'Arial', 'color': 'gray'})   
ax.text(0.303, plgnd_y-0.09, "Park id-number", **{'size':14, 'family': 'Arial'})
...
lab_props = {'fontsize': '24', 'family': 'Arial'}
...
# ======================
# Output name
outfp = os.path.join(outputdir, 'Finland_monthly_visitation_comparisons_selected_parks_2014.png')
